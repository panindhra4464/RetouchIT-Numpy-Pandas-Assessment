...
3. **Vectorized Operations**  
   ...
   *(Write the code)*  arr = np.array([-2,-1,0,1,2])
                       arr[arr<0]=(0)
                       arr
...
4. **Aggregation & NaN Handling**  
   ...
   ```  
   *(Write the code)*  matrix = np.array ([[1, np.nan, 3], [4, 5, np.nan]])
...
6. **DataFrame Creation & Basics**  
   ...
   ```  
   *(Write the code)*  data = {'Name':['Alice','Bob','Charlie'], 'Age':[25,30,35]}
                       ind = [100,101,102]
...
7. **Handling Missing Data**  
   ...
   ```  
   *(Write the code)*       df = pd.DataFrame({'A': [1, 2, np.nan], 'B': [np.nan, 5, 6]})
                            print(df)
                            meanB=df['B'].mean()
                            print(meanB)
...
8. **GroupBy & Aggregation**  
   ...
   ```  
   *(Write the code)*         df = pd.DataFrame({
                                               'Region': ['North', 'South', 'North', 'South'],
                                               'Sales': [100, 200, 150, 250],
                                               'Profit': [20, 30, 25, 40]})
...
9. **Merging DataFrames**  
   ...
   ```  
   *(Write the code)*                 orders = pd.DataFrame({'order_id': [1, 2], 'cust_id': [101, 102]})
                                      customers = pd.DataFrame({'cust_id': [101, 103], 'name': ['Alice', 'Bob']})
